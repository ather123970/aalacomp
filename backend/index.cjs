...
/* /api/ask handler moved to after Express `app` initialization to avoid referencing `app` before it's created. */

const app = express();
// CORS: allow credentials and restrict allowed origins (do NOT use wildcard when credentials are included)
...
const DEFAULT_ORIGINS = ['http://localhost:5173', 'http://127.0.0.1:5173', 'http://localhost:3000', 'http://127.0.0.1:3000', 'http://localhost'];
...
app.use(cors({
  origin: function(origin, callback) {
    // allow non-browser requests (e.g. server-to-server or same-origin tools) which have no origin
    if (!origin) return callback(null, true);
    // allow explicit configured origins
    if (FRONTEND_ORIGINS.indexOf(origin) !== -1) return callback(null, true);
    // allow common local network origins (mobile/dev device on same LAN)
    try {
      const u = new URL(origin);
      const h = u.hostname;
      // match IPv4 private ranges: 10.x.x.x, 192.168.x.x, 172.16-31.x.x
      if (/^10\./.test(h) || /^192\.168\./.test(h) || /^172\.(1[6-9]|2[0-9]|3[0-1])\./.test(h)) return callback(null, true);
      // allow hostnames that include 'local' or 'lan'
      if (h.includes('local') || h.includes('lan')) return callback(null, true);
    } catch (e) {
...
async function startServer() {
  try {
    // Accept multiple env names used across projects
  const MONGO_URI = process.env.MONGO_URI || process.env.MONGO || 'mongodb://127.0.0.1:27017/Aalacomputer' || null;
    if (MONGO_URI) {
      // Prevent mongoose from buffering commands when the DB is down (causes long timeouts).
      try {
        mongoose.set('bufferCommands', false);
      } catch (e) {}
      try {
        // serverSelectionTimeoutMS makes the driver fail fast when it can't reach the server
        await mongoose.connect(MONGO_URI, { connectTimeoutMS: 5000, serverSelectionTimeoutMS: 5000 });
        console.log('[db] MongoDB connected');
      } catch (e) {
        console.warn('[db] MongoDB connection failed:', e && (e.stack || e.message || e));
      }
    } else {
