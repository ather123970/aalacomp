...
const categories = ['Pre-Built', 'Custom Build', 'Parts', 'PC Accessories'];
...
export default function Navbar() {
  ...
  className="bg-panel text-primary fixed top-0 left-0 w-full z-[100101] shadow-sm"
      // ensure header has no transform style that could create stacking context
      // and explicitly allow pointer events in case other widgets overlap
            ...
            <AnimatePresence>
              {desktopCatOpen && (
                <FM.ul
                  ...
                  className="absolute right-0 mt-2 bg-card rounded-lg shadow-lg w-48 ring-1 ring-black/30 overflow-hidden"
                >
                  {categories.map((cat) => (
                    <FM.li key={cat} whileHover={{ scale: 1.02 }} className="cursor-pointer">
                      <button
        ...
        {isOpen && (
          ...
          >
              ...
              <li className="pt-1 border-t border-gray-800">
                ...
                <AnimatePresence>
                  {mobileCatOpen && (
                    <FM.ul initial="hidden" animate="visible" exit="exit" variants={mobileDropdownVariants} className="pl-3 mt-2 space-y-1 overflow-hidden">
                      {categories.map((cat) => (
                        <FM.li key={cat} whileHover={{ x: 6 }}>
                          <button onClick={() => handleCategoryClick(cat)} className="block w-full text-left px-3 py-2 hover:bg-blue-600 hover:text-white rounded transition">{cat}</button>
                        </FM.li>
                      ))}
                    </FM.ul>
                  )}
